import { Button, DropdownMenu, DropdownMenuCheckboxItem, DropdownMenuContent, DropdownMenuItem, DropdownMenuSeparator, DropdownMenuShortcut, DropdownMenuTrigger, Icons } from "@ui/index";
...
export function MenuButton() {
  ...
  const saveNodesAndEdges = useCallback((autoSafe = false) => {
  ...
  useEffect(() => {
    function handleKeyDown(event: KeyboardEvent) {
      if (event.key === 's' && (isAppleProduct ? event.metaKey : event.ctrlKey)) {
  ...
  return (
    <DropdownMenu open={dropDownOpen}>
      ...
      <DropdownMenuContent className="mr-4" onEscapeKeyDown={closeDropdown} onInteractOutside={closeDropdown}>
        ...
        <DropdownMenuCheckboxItem checked={autoSave} onClick={() => setAutoSave(!autoSave)}>
          Auto save

        ...
        >
            <TabsList className="w-full bg-transparent space-x-4">
              {features.map(feature => (
                <TabsTrigger key={feature.title} value={feature.title} className='data-[state=active]:bg-white data-[state=active]:text-blue-500 text-white text-xl rounded-full hover:bg-white/10'>
                  {feature.title}
                </TabsTrigger>
              ))}
            </TabsList>
            {features.map(feature => (
              <TabsContent key={feature.title} value={feature.title}>
                ...
                <div aria-hidden="true" className="w-2xl m-auto mt-8 overflow-hidden rounded-xl bg-slate-50 shadow-xl shadow-blue-900/20">
                  <Image
                    className="w-full"
                    src={feature.image}
                    alt=""

...
impl Component for Led {
    ...
    fn update_config(&mut self, config: serde_json::Value) -> Result<(), String> {
        let new: LedConfig = serde_json::from_value(config).map_err(|e| format!("Invalid config: {}", e))?;
    ...
    fn destroy(&mut self) { self.stop_blink(); let _ = self.turn_off(); self.board = None; }

...
fn default_pin() -> u8 { 6 }
fn default_length() -> u16 { 32 }
fn default_color_order() -> String { "GRB".to_string() }
...
impl Pixel {
    ...
    pub fn new(id: String, config: PixelConfig) -> Self {
        ...
        Self {
            base: ComponentBase::new(id, ComponentValue::Array(
    ...
    fn send_config(&self) -> Result<(), crate::error::RuntimeError> {
        if let Some(board) = &self.board {
            ...
            board.sysex(PIXEL_COMMAND, data).ignore();
        }
        Ok(())
    }
    ...
    fn send_show(&self) -> Result<(), crate::error::RuntimeError> {
        if let Some(board) = &self.board {
            board.sysex(PIXEL_COMMAND, vec![PIXEL_SHOW]).ignore();
        }
        Ok(())
    }
    ...
    fn apply_preset(&mut self, index: usize) -> Result<(), crate::error::RuntimeError> {
    ...
    /// Turn off all pixels
    fn off(&mut self) -> Result<(), crate::error::RuntimeError> {
        self.send_set_strip(0)?;
        self.send_show()?;
        self.pixels.fill(0);
        self.update_value();
        Ok(())
    ...
    fn dispatch(&mut self, method: &str, args: ComponentValue) -> Result<(), crate::error::RuntimeError> {
        match method {
            "value" => {
                ...
                let clamped = if self.config.presets.is_empty() { 0 } else { index.min(self.config.presets.len() - 1) };
                self.apply_preset(clamped)?;
            ...
            "color" => {
                ...
                match args {
                    ComponentValue::String(hex) => {
                        ...
                        let color = Self::parse_hex_color(&hex);
                        self.send_set_strip(color)?;
                        self.send_show()?;
                        self.pixels.fill(color);
                        self.update_value();
                    }
                    ComponentValue::Array(arr) => {
                        let colors: Vec<String> = arr.iter().filter_map(|v| {
                            if let ComponentValue::String(s) = v { Some(s.clone()) } else { None }
                        }).collect();
                        self.apply_colors(&colors)?;
                    }
                    _ => {
            ...
            "set" => {
                ...
                self.send_shift(amount.max(1), forward, true)?;
                self.send_show()?;
            ...
            _ => Err(crate::error::RuntimeError::ComponentError(
                format!("Pixel: unknown method '{method}'")
            )),
        }
    }

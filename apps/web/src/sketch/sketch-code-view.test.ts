...
import type { SketchInvoker } from "@/sketch/codegen/sketch-protocol";
import {
...
describe("projectSketchResult", () => {
  ...
  test("the full sketch text is preserved verbatim for copying", async () => {
    ...
    const invoker: SketchInvoker = async () => ({ success: true, data: { sketch, problems: [] } });
  ...
  test("an empty Flow yields the generator's empty sketch with no error", async () => {
    ...
    const invoker: SketchInvoker = async (command) => {
      ...
      return { success: true, data: { sketch: emptySketch, problems: [] } };
    };
  ...
  // Scenario: An error-severity problem blocks emission — problems surface as text
  test("blocking problems are surfaced as an error block, not a sketch", async () => {
    const invoker: SketchInvoker = async () => ({
      success: true,
      data: {
        sketch: null,
        problems: [
          {
            nodeId: "led-1",
            nodeType: "Led",
            message: "Node led-1 (Led) shares the generated identifier 'led_1' with led_1",
            severity: "error",
          },
        ],
      },
    ...
    expect(state.value).toContain("led-1");
    expect(state.value).toContain("identifier");
    expect(state.value).not.toContain("void setup()");
  });

  // Scenario: Warnings ride alongside the sketch as a leading comment block
  test("warnings are prepended as comments and the sketch still renders", async () => {
        ...
        problems: [
          {
            ...
            nodeType: "Mqtt",
            message:
              "Node mqtt-1 (Mqtt) requires networking, which board target 'Arduino Uno' does not offer — the generated network code assumes an ESP32-class board",
            severity: "warning",
          },
        ],
      },
    ...
    expect(state.value).toContain("// Board warnings");
    expect(state.value).toContain("networking");
    expect(state.value).toContain("void setup()");
    // Warnings lead; the sketch follows intact.
    expect(state.value.endsWith(sketch)).toBe(true);
  });

  // Scenario: The header carries the generation timestamp under the version line
  test("a Generated-by header gains a Generated-at line with the injected time", async () => {
    const sketch = "// Generated by microflow v0.10.3.\nvoid loop() {}";
    const invoker: SketchInvoker = async () => ({ success: true, data: { sketch, problems: [] } });

    const state = await projectSketchResult(
      invoker,
      [NODE],
      [],
      undefined,
  ...
  test("the selected target id is forwarded in the command", async () => {
    ...
    const invoker: SketchInvoker = async (command) => {
      ...
      return { success: true, data: { sketch: "ok", problems: [] } };
    };
...
describe("createDebouncedRegenerator", () => {
  ...
  test("a graph change triggers regeneration after the debounce window", async () => {
    ...
    const invoker: SketchInvoker = async () => ({ success: true, data: { sketch: "sketch-v1", problems: [] } });
  ...
  test("rapid successive edits collapse into one regeneration with the latest graph", async () => {
    ...
    const invoker: SketchInvoker = async (command) => {
      ...
      return { success: true, data: { sketch: `nodes:${command.flow.nodes.length}`, problems: [] } };
    };
    const results: SketchViewState[] = [];
  ...
  test("an unchanged graph is not regenerated (skips redundant generation)", async () => {
    ...
    const invoker: SketchInvoker = async () => {
      calls++;
      return { success: true, data: { sketch: "x", problems: [] } };
    };
  ...
  test("a stale in-flight response is dropped; only the latest result is applied", async () => {
    ...
    const invoker: SketchInvoker = () =>
      new Promise((resolve) =>
        resolvers.push((value) => resolve({ success: true, data: { sketch: value, problems: [] } })),

...
import type { GenerateSketchCommand, SketchInvoker } from "@/sketch/codegen/sketch-protocol";
 ...
 * comment block for the editor. Each problem names its Node and the constraint
 * it violated, so the Author sees exactly why no sketch was emitted.
 */
function formatProblems(problems: ValidationProblem[]): string {
  const header = "// Cannot generate a sketch:";
  const lines = problems.map((p) => `// - ${p.message}`);
  return [header, ...lines].join("\n");
}

/**
 * Render warning-severity problems as a comment block prepended to the sketch.
 * Warnings never block emission; keeping them as C++ comments means the
 * displayed text stays a valid, downloadable sketch that carries its own
 * board-fit caveats.
 */
function formatWarnings(problems: ValidationProblem[]): string {
  const header = "// Board warnings — the Flow may not fit the selected board:";
  const lines = problems.map((p) => `// - ${p.message}`);
  return [header, ...lines].join("\n");
}
...
export type SketchViewState = {
  ...
  isError: boolean;
  /** Present (true) when the sketch carries board-fit warnings. */
  hasWarnings?: boolean;
};

/**
 * Insert a `// Generated at:` line under the generator's header. The generation
 * time is host state, not a function of the Flow, so it is stamped here rather
 * than in the (deterministic) Rust generator. Sketches without the header line
 * (e.g. test fixtures) pass through untouched.
 */
export function stampGeneratedAt(sketch: string, generatedAt: Date): string {
  return sketch.replace(
    /^(\/\/ Generated by microflow[^\n]*)/,
...
export async function projectSketchResult(
  ...
  // No sketch means an error-severity problem blocked emission — render the
  // problems as an error block. Otherwise the sketch renders verbatim, with
  // any warnings prepended as a comment block (still a valid sketch).
  const { sketch, problems } = response.data;
  if (sketch === null) {
    return { value: formatProblems(problems), isError: true };
  }
  const stamped = stampGeneratedAt(sketch, generatedAt);
  if (problems.length > 0) {
    return { value: `${formatWarnings(problems)}\n\n${stamped}`, isError: false, hasWarnings: true };
  }
  return { value: stamped, isError: false };
}

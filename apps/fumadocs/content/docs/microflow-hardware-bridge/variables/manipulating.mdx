...
<Clip title="A value crossing in both directions" duration="0:25" spec="bridge-both-ways">
The value crosses in both directions without either side being reconfigured.
</Clip>
...
MQTT routes messages by topic. Each variable has its own topic, built from your identifier and the variable's ID in Figma.

To set a variable, publish to:

```
microflow/xiduzo/my-app/variable/1-25/set
```

| Segment | Meaning |
| --- | --- |
| `microflow` | The root for every Microflow topic |
| `xiduzo` | Your configured [identifier](/docs/microflow-hardware-bridge/variables#mqtt-settings) |
| `my-app` | A name for your client. Any string without spaces. |
| `variable` | The message type |
| `1-25` | The Figma variable ID, with `VariableID:` removed and each `:` replaced by `-` |
| `set` | The action |

The plugin also publishes the full variable list, retained, to `microflow/xiduzo/figma/variables`. Read that topic to discover the IDs.
...
To receive a variable's value, subscribe to:

```
microflow/xiduzo/figma/variable/1-25
```
...
| `figma` | The message comes from the Figma plugin |
| `variable` | The message type |
| `1-25` | The Figma variable ID, with `VariableID:` removed and each `:` replaced by `-` |

---
title: Motion
description: Detect movement using a PIR motion sensor for security systems or automatic triggers.
---
...
<Clip title="Motion" duration="0:25" spec="motion">
The LED lights on movement and stays lit for the sensor's own hold time before releasing.
</Clip>
...
<Callout type="info" title="Image placeholder">
  Diagram of a PIR sensor board highlighting the two adjustment knobs: the sensitivity knob and the delay time knob, with arrows indicating which direction to turn for more or less sensitivity/delay.
</Callout>
...
<Callout type="info" title="Image placeholder">
  Diagram of a PIR sensor board showing the mode jumper, with labels for single trigger mode (L) and repeat trigger mode (H/B).
</Callout>
...
- **`event`** — Fires when motion starts and when it stops.
- **`true`** — True while motion is detected.
- **`false`** — True while nothing is moving.
- **`value`** — `1` while motion is detected, `0` otherwise.

## Settings

- **Controller** — Which hardware controller the component uses. Default `HCSR501`.
- **Pin** — Which pin the component is connected to. Default `8`.

## Resources

- [Johnny-Five — the API this node’s behaviour is modelled on](https://johnny-five.io/examples/ir-motion/)

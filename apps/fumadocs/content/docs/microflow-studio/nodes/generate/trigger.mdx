...
The Trigger node detects when values change in a specific way, rather than checking if they match an exact number (which is what the [Compare](/docs/microflow-studio/nodes/decide/compare) node does).

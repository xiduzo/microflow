...
import { type MessageType, sendToPlugin } from "../../common/messages";
 ...
 *
 * Key difference from Figma plugin: Penpot delivers messages directly
 * as `event.data` (no `pluginMessage` wrapper).
 */
export function useMessageListener<T = undefined>(
  ...
  useEffect(() => {
    function handler(event: MessageEvent) {
      ...
      callbackRef.current(event.data.payload as T);
    }

...
| `schema.ts` (`FlowDocument`) | `syncedstore`, `valtio-yjs`, `zustand-middleware-yjs` | **Kept the seam, fixed the shape.** Those libraries impose a state-management paradigm on a codebase that already has one, and none would have fixed the actual problem: nodes were stored as plain objects, so Yjs could not merge them. [ADR-0017](0017-nested-node-fields-for-concurrent-edits.md) nested the fields instead — concurrent edits now merge, and the wrapper stayed. |

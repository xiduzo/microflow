...
2. **Microflow devtools** — an app-wide, TanStack-style bottom-drawer debug console (floating launcher bottom-right, raised above the TanStack devtools), streaming the **whole app's** activity through one unified `useDevLogStore`: backend `log::` records (hardware / MQTT / LLM — forwarded to the webview via `tauri-plugin-log`'s `log://log` webview target, `src-tauri/src/lib.rs`) **and** flow component-events. Rows: time · level · source · message, with filter + pause + clear. The two flow-ingest paths (browser `flow-reactor.ts`, desktop `use-component-events.ts`) were unified behind one shared `applyComponentEvent` (`apps/web/src/runtime/event-ingest.ts`) so node values, edge animations, and the dev-log stay in lock-step on both platforms. New: `devtools/dev-log.ts`, `devtools/use-backend-logs.ts` (parses the `[date][time][target][LEVEL] msg` format → source category), `devtools/microflow-devtools.tsx`, `devtools/ui-panel.ts`, `runtime/format-value.ts`; mounted at the app root (`routes/__root.tsx`) beside the TanStack devtools. Verified live in the running desktop app + web typecheck/oxlint clean.

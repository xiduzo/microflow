...
fn golden_sketches_match() {
    ...
    for golden in corpus() {
        ...
        if strip_version(&sketch) != strip_version(&expected) {
...
/// The header carries the crate version, which every release bumps. Comparing it
/// would make all twelve goldens "change" on a version bump and nothing else —
/// noise that says nothing about the generated code.
fn strip_version(sketch: &str) -> String {
    sketch
        .lines()
        .filter(|line| !line.starts_with("// Generated by microflow v"))
        .collect::<Vec<_>>()
        .join("\n")
}

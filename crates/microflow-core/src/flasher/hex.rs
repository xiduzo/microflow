//! Intel HEX file parser — pure, transport-free, shared by the desktop flasher
//! and the browser (via wasm). Parses the compiled `.hex` (`StandardFirmata` or a
//! user sketch) into the raw flash image the bootloader protocols program.

use super::FlashError;

/// Parse Intel HEX format into raw binary data, gaps filled with `0xFF`.
///
/// # Errors
/// Returns [`FlashError::InvalidHex`] if a data record is truncated.

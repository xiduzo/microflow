//! Test support shared by the cloud nodes (`Mqtt`, `Llm`, `Figma`) and the
//! host-peripheral nodes that share their sans-IO shape (`Midi`, `Music`). Their
//! [`Component`] impls live with each node, in `crate::nodes::<node>::runtime`,
//! behind this same `cloud` feature (ADR-0026).
//!

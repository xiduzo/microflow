...
use super::config::{fold_bytes, ByteDecode, I2cDeviceConfig, OutputFormat};
use crate::runtime::{
...
/// arithmetic lives once in the sibling `config`; the sketch emitter
/// (`nodes/i2c_device/codegen.rs`) transcribes the same descriptor to C++, and
...
impl Component for I2cDevice {
    ...
    fn board_wiring(&self) -> BoardWiring {
        BoardWiring {
            ...
            i2c_read_delay_us: super::config::is_no_hold_sht2x(&self.config.device)
                .then_some(16_000),
...
mod tests {
    ...
    // hold-master remap) is unit-tested authoritatively in `crate::nodes::i2c_device::config`.
    ...
    // (the fold arithmetic itself is unit-tested in `nodes/i2c_device/config.rs`) ---

//! `I2cDevice` emitter — mirrors `nodes/i2c_device/runtime.rs`.
//!
...
use super::config::{ByteDecode, I2cDeviceConfig, OutputFormat};
...
pub(crate) fn emit(node: &FlowNode, inputs: &NodeInputs) -> NodeEmission {
    ...
    for &payload in super::config::device_init_writes(&config.device) {
...
/// Delegates to the shared [`super::config::effective_register`] (see
...
fn effective_register(config: &I2cDeviceConfig) -> u8 {
    super::config::effective_register(&config.device, config.address, config.register)
}
...
/// shared ([`super::config::is_no_hold_sht2x`]); the magnitude is
...
fn read_delay_ms(config: &I2cDeviceConfig) -> u32 {
    if super::config::is_no_hold_sht2x(&config.device) { 30 } else { 0 }
}

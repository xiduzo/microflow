...
//! (`nodes/i2c_device/runtime.rs`, interpret) and the codegen emitter
//! (`nodes/i2c_device/codegen.rs`, compile to C++).
//!
...
//! which drifted (see `nodes/i2c_device/codegen.rs`).
...
/// (`nodes/i2c_device/runtime.rs` → [`fold_bytes`], producing a `ComponentValue`)
/// and the sketch emitter *transcribes* it to a C++ fold
/// (`nodes/i2c_device/codegen.rs`, producing a `long`). The two languages cannot

...
/// True when `node_type` names a browser-only Node (no board equivalent).
fn is_browser_only_node(node_type: &str) -> bool {
    BROWSER_ONLY_NODE_TYPES.contains(&node_type)
}
...
pub fn emit(node: &FlowNode) -> NodeEmission {
    ...
    let comment = match node.node_type.as_deref() {
        ...
        Some(kind) if is_browser_only_node(kind) => format!(
            "// unsupported Node {token} ({kind}): browser-only Node — no Arduino hardware equivalent, no code generated"
        ),
        Some(kind) => format!(
...
mod tests {
    ...
    fn audio_player_emits_browser_only_placeholder() {
        let e = emit(&node("audio-1", Some("AudioPlayer")));
        assert_eq!(e.declarations.len(), 1);
        let comment = &e.declarations[0];
        assert!(comment.contains("audio_1"), "identifies the Node id");
        assert!(comment.contains("AudioPlayer"), "identifies the Node type");
        assert!(
            comment.contains("browser-only"),
            "explains it is browser-only, got: {comment}"
        );
        assert!(!comment.contains("networked target"), "not a Cloud message");
        assert!(e.setup.is_empty());
        assert!(e.loop_body.is_empty());
    }

...
/**
 * A peer that has not joined yet. `connection` only exists once `join`
 * resolves, so it is absent here rather than asserted into place with a cast
 * — those casts claimed a value that did not exist and hid the gap from the
 * type checker.
 */
type PendingPeer = Omit<Peer, "connection"> & { connection?: RoomConnection };

function newPeer(): PendingPeer {
  return { received: [], closed: false };
}

function socketFor(peer: PendingPeer, bufferedAmount?: () => number): Connection {
  return {
...
describe("YjsServer connection limits", () => {
  test("a peer whose send buffer is over the limit is dropped, not buffered", async () => {
    ...
    const stalled = newPeer();
    const healthy = newPeer();
  ...
  test("a flooding connection is throttled once its bucket empties", async () => {
    ...
    const flooder = newPeer();
    const observer = newPeer();

...
- `apps/web/scripts/codegen-node-registry.ts` reads `entries` from the catalog **and** the Port/Emit sets from `apps/web/wire-interface.generated.json`, then writes three files under `apps/web/src/nodes/`: `component-types.generated.ts`; `catalog.generated.ts` (`NODE_CATALOG` — per-type `defaults`, `schema` and host `adapter`; React-free, for non-UI code); and `node-types.generated.ts` (`NODE_TYPES` — the React component per type, for ReactFlow). Run via `bun run codegen` in `apps/web`.
...
- **Browser** — `apps/web/src/runtime/flow-reactor.ts`. A `setTimeout` / Web-Serial loop driving the `microflow-runtime-wasm` engine: writes `outbound_bytes` to the port, fans `component_events` to the Zustand stores, maps `wakeups` to `setTimeout` and `cancellations` to `clearTimeout`. The serial/timer/effects-apply half is a shallow pass-through (depth lives behind the wasm seam in Rust); the **cloud** half is its own [Browser CloudPerformer](#browser-cloudperformer), and every call into the engine goes through the [RuntimeBridge](#runtimebridge).

## Detection codec

The `FirmataSession` instance the probe drives, in `apps/web/src/board/web-serial.ts` ([ADR-0018](docs/adr/0018-two-firmata-decoders.md)). It answers the **handshake** questions — what firmware is this, what pins does it have — which is what `tryConnectAtBaud` polls via `firmwareName()` and `pinsJson()`, and it is the only decoder alive during the probe, before any [Runtime Host](#runtime-host) exists. It supplies the runtime's [pin seed](#pin-seed) and keeps `connectedState` truthful.

In steady state the browser feeds each inbound chunk to **both** the detection codec and the runtime, unfiltered and in the same order. That is not duplication to be eliminated: both wrap the same `microflow_core::firmata::FirmataClient`, so they are one codec answering two questions at two lifetimes, and consistency follows from the identical stream rather than from any synchronisation between them. The detection codec's per-chunk report is not decoded on the JS side — nothing consumes it once a flow is running.

## Pin seed

The one-shot `setPins` at `FlowReactor.attach` ([ADR-0018](docs/adr/0018-two-firmata-decoders.md)). It hands the runtime the capability response it was not alive to decode, because the [detection codec](#detection-codec) consumed it during the probe. One shot suffices: boards do not send capability responses unsolicited, and a re-enumeration re-probes and re-attaches, seeding again.

## RuntimeBridge

The browser's one crossing into the wasm flow runtime, and the only holder of a `FlowRuntime` handle. Lives in `apps/web/src/runtime/runtime-bridge.ts` ([ADR-0017](docs/adr/0017-wasm-fault-seam.md)). The surface is one **typed entry point** per runtime op the host uses — `setPins`, `updateFlow`, `wake`, `feedBytes`, `dispatch`, `injectEvent`, `deliverMessage`, `reconcileSubscriptions`, `midiListeners` — each owning its op's whole call codec: the JSON encode of arguments and the decode of replies into the ts-rs binding types ([`Effects`](#effects), `DesiredSub[]`, `MidiListener[]`), so no caller touches a JSON string or casts a result. `call(op, node, fn)` is the **private** core they all route through: every wasm entry point returns `Result<String, JsError>` — a *throw* on the JS side — so `call` wraps each one, returns the reply or `undefined`, and **never throws**. Faults are classified into three kinds and routed to a surface that already exists: `badInput` (Rust returned `Err` — or the host could not decode the reply; the module is intact and only this turn is lost) → the node's diagnostic badge when the op names a node; `engineBroken` (a wasm trap) → the board error state, reactor disposed; `disposed` → the console, once.
...
into `apps/web/src/cloud/cloud-performer.ts` — the host-free twin of
...
no broker, no runtime (`cloud/cloud-performer.test.ts`). `FlowReactor`
...
the same transport (`ai/llm-client.ts`, with latest-wins
...
per-broker connection manager in `apps/web/src/cloud/`).
...
A stored connection to any backend that can run an LLM completion — since [ADR-0021](docs/adr/0021-one-llm-transport-in-the-webview.md) a **webview** concept in both hosts, not a Rust capability. A provider config's connection half is `LlmProviderConn` — `{ kind?: "http" | "cli", baseUrl, apiKey }` — and `adapterFor(provider, model)` in `apps/web/src/ai/adapter.ts` is the one place a config becomes a TanStack AI adapter, resolved by both AI surfaces: the `Llm` flow node's transport (`ai/llm-client.ts`, driven by the [Browser CloudPerformer](#browser-cloudperformer)) and Ask AI (the [Turn Runner](#turn-runner)). `"http"` speaks the OpenAI chat-completions wire shape (OpenAI, OpenRouter, LM Studio, llama.cpp, vLLM and Ollama all serve it) through `hostFetch()` (`ai/endpoint.ts`) — the page's `fetch` in a browser, the Tauri HTTP plugin's in the desktop webview, which is the entire CORS story. `"cli"` runs a local agent CLI as a subprocess (`ai/cli-adapter.ts` / `cli-providers.ts`) — desktop only, and excluded from Ask AI even there; both exclusions are answered by [Host Limitation](#host-limitation). The desktop actor performs no LLM I/O: `Actor::perform_cloud` forwards an `LlmGenerate` [`CloudRequest`](#cloudrequest) to the webview as an `llm-request` event, and the result re-enters through the `llm_result` command onto `ActorMsg::Inject`.

## Turn Runner

One Ask AI turn as a plain async generator — `runTurn(options)` in `apps/web/src/ai/turn-runner.ts`. Owns everything between "the user hit send" and "the reply is final": filtering the transcript's own error notices out of the history sent to the model (they are UI, not something the model said), driving `@tanstack/ai`'s `chat()` against the flow tools, mapping the stream to `TurnUpdate` patches (text so far, tools run so far, or the failure that ended it), the no-provider notice, and `MAX_ITERATIONS` (12 model↔tool round trips — enough for "add a button that toggles an LED" plus a correction, without letting a confused small model loop forever). It never throws: every failure becomes an `error: true` update, and an abort ends the turn silently with the partial text. Returns what `confirm` mode staged, as `PendingChange[]`; `mergePending` appends in staging order (entries are uid-keyed apply-thunks, and `applyChanges` runs the whole set in one transaction, so a later change to the same node wins by ordering — one undo step). The [LLM Provider](#llm-provider) adapter is taken as a **lazy resolver** at the `adapterFor` seam ([ADR-0021](docs/adr/0021-one-llm-transport-in-the-webview.md)) — a resolution failure is this turn's error like any other, not a new seam. No React inside: `use-ask-ai.ts` is state plumbing over it, and the whole loop is testable against a fake adapter (`kind`/`name`/`model`/`chatStream`/`structuredOutput` suffice).

## Flow Tool Server

Ask AI's tools, published where an agent CLI can call them — `withFlowToolServer` in `apps/web/src/ai/mcp-bridge.ts` over the MCP server in `apps/web/src-tauri/src/mcp/` ([ADR-0024](docs/adr/0024-flow-tools-over-mcp-for-agent-clis.md)). A print-mode CLI has no wire format for `chat()`'s tools and runs its own agent loop, so the tools are not serialised into its prompt; they are published on a unix socket (a named pipe on Windows) that the CLI reaches by spawning `microflow --mcp <token>` — the same binary in a mode that returns before Tauri boots. Every `tools/call` is a round trip back up into the webview, because the tools mutate the Yjs `FlowDocument` and a Rust copy would be a second definition of every one of them. Exposure is a **session**, not a server: `withFlowToolServer` opens one for the length of a turn and closes it in a `finally`, an idle app answers `tools/list` with `[]`, and a client that does not present the session token sees the same nothing. The agent loop being inside the CLI is what the bounds are for — a 40-call budget, a 30-second deadline per call, close-fails-everything-in-flight, and one live session so a broadcast event cannot apply a write twice. Abort closes the session rather than killing the process: the CLI keeps running, and can no longer reach the flow. `CliProvider.mcpArgs` (`ai/cli-providers.ts`) says how one CLI is pointed at a server *for a single run*, and its absence is what [Host Limitation](#host-limitation) reports as "no flow tools".
...
Frontend mirror of **Wiring**. Each node folder may hold a `<node>.adapter.ts` that exports an `adapter: NodeHostAdapter` (see `apps/web/src/nodes/_base/host-adapter.ts`) describing what the host store + global hotkey listener need from this node:
...
`apps/web/src/nodes/_base/node-handles.tsx`. `<NodeHandles
...
`readOnlyDocument(doc)` in `apps/web/src/session/read-only-document.ts` — a `FlowDocument` proxy whose mutating methods (`addNode`, `setMeta`, `undo`, …) are no-ops. One write-guard for every call site that reaches the doc directly (node dialogs, import/export, history, board-target picker) instead of a `readOnly` check at each, which the next call site would have to remember. Getters, observers and the raw `doc`/`nodes`/`edges` handles pass through, so incoming remote updates and the Yjs→React read path are unaffected. `ReactFlowBridge` writes through `doc.nodes` rather than these methods and carries its own flag; the two guards are independent on purpose.
...
The handle `YjsServer.join(flowId, socket, userId, canWrite)` hands back, in `packages/collab/src/yjs-server.ts` — `{ flowId, userId, canWrite, receive, close }`. The only way to reach a room: bytes go in through `receive`, the attachment ends with `close`. A transport cannot address a room by id plus a socket it claims to own, so the room always recognises the sender and its access decision. The handle also owns the awareness client ids seen on that socket, which `close` clears.

**Access** is the room's own vocabulary for what a user may do — `"none" | "read" | "write"` — deliberately not [Flow Role](#flow-role): the collab package knows about reading and writing, not owners and collaborators, and the caller maps between them. `setAccess(flowId, userId, access)` applies a change to every live connection that user holds; `"none"` closes their sockets.

Presence is client-authored, so the room rewrites `user.id` on every inbound awareness state to the authenticated user behind the connection before applying it — otherwise a Viewer can present themselves as the Owner in cursors and the collaborator panel.

## RoomStore

The persistence seam under a Yjs room: `{ load(flowId), save(flowId, state) }` in `packages/collab/src/room-store.ts`. `YjsServer` owns rooms, awareness and the debounce policy; where the document lives sits behind this interface. Two adapters: `drizzleRoomStore` (`flow.ydoc`, the only file in the package that imports the database) and `MemoryRoomStore` (records every `save`, so room lifecycle and the flush-on-last-disconnect are testable without one). `handler.ts` is the composition root that pairs the singleton server with the production adapter.

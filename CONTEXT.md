...
Ask AI's tools, published where an agent CLI can call them — `withFlowToolServer` in `apps/web/src/lib/ai/mcp-bridge.ts` over the MCP server in `apps/web/src-tauri/src/mcp/` ([ADR-0024](docs/adr/0024-flow-tools-over-mcp-for-agent-clis.md)). A print-mode CLI has no wire format for `chat()`'s tools and runs its own agent loop, so the tools are not serialised into its prompt; they are published on a unix socket (a named pipe on Windows) that the CLI reaches by spawning `microflow --mcp <token>` — the same binary in a mode that returns before Tauri boots. Every `tools/call` is a round trip back up into the webview, because the tools mutate the Yjs `FlowDocument` and a Rust copy would be a second definition of every one of them. Exposure is a **session**, not a server: `withFlowToolServer` opens one for the length of a turn and closes it in a `finally`, an idle app answers `tools/list` with `[]`, and a client that does not present the session token sees the same nothing. The agent loop being inside the CLI is what the bounds are for — a 40-call budget, a 30-second deadline per call, close-fails-everything-in-flight, and one live session so a broadcast event cannot apply a write twice. Abort closes the session rather than killing the process: the CLI keeps running, and can no longer reach the flow. `CliProvider.mcpArgs` (`lib/ai/cli-providers.ts`) says how one CLI is pointed at a server *for a single run*, and its absence is what [Host Limitation](#host-limitation) reports as "no flow tools".
